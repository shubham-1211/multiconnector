...
function readFile(dir, ext, collection, file) {
    return new Promise(function (resolve, reject) {
        fs.readFile(dir + '/' + file, 'utf8', function (err, data) {
            if (err) return console.log('error', 'File read error', err.message);
            try {
                switch (ext) {
                    /** JSON. */
                    case '.json':
                ...
                console.log('info', 'Loaded ' + dir + '/' + file + '.');
            } catch (err) {
...
function loadJSON(collection, string) {
    try {
        ...
        for (let i = 0; i < Object.keys(object).length; i++) {
            ...
            console.log('info', 'Loaded from environment ' + collection + '/' + filename + '.');
        }
    } catch (err) {
        console.log('error', err.message);
    }
...
const composeOutput = async (template, input) => {
    ...
    console.log('input' + template.protocol);
