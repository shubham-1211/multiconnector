...
const getData = async (config, pathArray) => {
    ...
    try {
        ...
        if (!fs.existsSync(WSDLDir)) {
            ...
            console.log('info', 'Created folder for WSDL files.');
        }
        ...
        try {
            ...
            console.log('error', 'Failed to parse WSDL from config.');
        }

        // Download WSDL file, if it does not exist.
        if (fs.existsSync(WSDLFile)) {
            console.log('info', 'Initiated SOAP connection ' + config.productCode);
            items = await createSOAPClient(config, WSDLFile, pathArray);
            console.log('info', 'Closed SOAP connection ' + config.productCode);
        } else {
            console.log('info', 'Started downloading WDSL file...');
            ...
            console.log('info', 'Download finished.');

            await writeSOAPFile(WSDLFile, downloadedWSDLFile);

            if (downloadedWSDLFile) {
                console.log('info', 'Downloaded ' + encodeURI(config.productCode) + '.xml');
                items = await createSOAPClient(config, WSDLFile, pathArray);
            } else {
                console.log('info', 'Failed to download ' + encodeURI(config.productCode) + '.xml');
            }
        }
