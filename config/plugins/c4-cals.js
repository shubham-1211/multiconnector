            ...
            if (Object.keys(config.dataPropertyMappings).includes('OrderInformation')) {
                ...
                if (Object.hasOwnProperty.call(value, 'vendor')) {
                    ...
                    try {
                        ...
                        console.log('info', 'Store CALS identifiers from sent order.');
                        // console.log('info', 'orderNumberToCALSId: ' + JSON.stringify(orderNumberToCALSId));
                        // console.log('info', 'orderIdToCALSInstanceId: ' + JSON.stringify(orderIdToCALSInstanceId));
                    } catch (e) {
...
const controller = async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            console.log('info', 'Received trigger request from ' + (req.get('x-real-ip') || req.get('origin') || req.socket.remoteAddress));
        ...
        try {
            ...
            console.log('info', '1. Query self with REST path /instances/${instanceId}/' + resource + '/${entityId}');
        ...
        try {
            ...
            if (req.body.isTest) {
                ...
                console.log('info', '2. Skip sending to ' + vendorProductCode + ', isTest=' + req.body.isTest);
            } else {
                template = await connector.resolvePlugins(template);
                template.config = config;
                console.log('info', '2. Send received data to vendor data product ' + vendorProductCode);
...
const template = async (config, template) => {
    try {
        if (Object.hasOwnProperty.call(template.parameters.targetObject, 'vendor')) {
            ...
            console.log('info', 'Connector consumes CALS REST API ' + resource + ' by entityId ' + template.authConfig.path);
            ...
            console.log('info', 'Include headers from trigger request' + ', '
        ...
        if (Object.hasOwnProperty.call(template.parameters.targetObject, 'sender')) {
            /** CALS connector */
            console.log('info', 'Received produced data from '
            ...
            try {
                ...
                console.log('info', 'Resolved ' + data.PurchaseOrderNumber + ' to PurchaseOrderId ' + data.PurchaseOrderId);
                console.log('info', 'Resolved ' + data.PurchaseOrderId + ' to InstanceId ' + data.InstanceId);
                ...
                console.log('info', 'Body: ' + JSON.stringify(data));
                ...
                console.log('info', 'Include headers from received broker request' + ', '
                    + 'x-is-test: ' + config.static.headers['x-is-test']);
                console.log('info', '3. Send data to URL ' + config.static.url);
