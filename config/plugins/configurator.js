...
const requestConfigs = async (configs = []) => {
    ...
    for (let i = 0; i < configs.length; i++) {
        try {
            ...
            console.log('info', 'Received configs from ' + current + ': ' + result.message.map(o => o.productCode));
        } catch (err) {
            console.log('error', 'Failed to request configs from ' + current + ': ' + err.message);
        }
    }
};
        ...
        if (config !== null) {
            try {
                ...
                if (!systemTemplates.includes(template)) {
                    ...
                    console.log('info', 'Update ' + productCode);
            ...
            /** 2. Remove. */
            console.log('info', 'Remove ' + productCode);
