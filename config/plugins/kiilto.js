    ...
    try {
        ...
        */
        ...
        try {
            ...
            console.log('info', '1. Query self with path ${targetObject.idLocal} as ' + req.body.filename);
        ...
        try {
            ...
            console.log('info', 'Set receiver data product as ' + receiverProductCode + '.');
        }
        ...
        try {
            ...
            console.log('info', '2. Send received data to receiver data product ' + receiverProductCode + ', isTest=' + req.body.is_test);
            ...
            try {
                ...
                result.output.data.order.map((order) => {
                    ...
                    order.deliveryLine = order.deliveryLine.map((l) => {
                        ...
                        console.log('info', 'Changed ' + l.product.codeProduct + ' to ' + productCodeToCALSId[order.idSystemLocal][l.product.codeProduct]);
                        return {
                            ...l,
    ...
    try {
        if (Object.hasOwnProperty.call(template.parameters.targetObject, 'sender')) {
            ...
            try {
                ...
                console.log('info', 'Store CALS identifiers from received order.');
                console.log('info', 'orderNumberToCALSId: ' + JSON.stringify(orderNumberToCALSId));
                console.log('info', 'productCodeToCALSId: ' + JSON.stringify(productCodeToCALSId));
...
const stream = async (template, data) => {
    ...
    for (let i = 0; i < data.length; i++) {
        try {
            ...
            console.log('info', '1. From: ' + template.productCode);
            ...
            if (template.plugins.find(p => p.name === 'broker') && template.config.plugins.broker) {
                ...
                console.log('info', '2. Send received data to receiver data product ' + receiverProductCode + ', isTest=' + false);
